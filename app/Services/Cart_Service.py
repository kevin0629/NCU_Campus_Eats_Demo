...
# 4.1 新增餐點進購物車Service
def add_item_to_cart_service(db_session, item_id, item_price, customer_id):
...
# 4.2 查看購物車內容
def get_cart_items_service(db_session, customer_id):
    return fetch_cart_item(db_session, customer_id)

# 4.3 移除購物車一個商品
def remove_item_from_cart_service(db_session, order_id, order_detail_id):
